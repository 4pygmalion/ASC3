...
Example:
    $ python3 results/revision/1_4_train_boostrapping.py \
        -d /data2/heon_dev/repository/3ASC-Confirmed-variant-Resys/dataset_positive_negative.pickle \
        --run_name revision_boostrapping \
        --n_epochs 100 \
        --instance_loss_weight 10 \
        --device 3

    $ python3 experiments/1_4_train_boostrapping.py \
        -d /data2/heon_dev/repository/3ASC-Confirmed-variant-Resys/dataset_positive_negative.pickle \
        --run_name revision_boostrapping \
        --n_epochs 100 \
        --instance_loss_weight 10 \
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="Instance loss weight (default: 10)",
    )
    ...
    with mlflow.start_run(experiment_id=exp_id, run_name=RUN_NAME) as parent_run:
        ...
        for fold in range(1, 31):
            with mlflow.start_run(
                ...
                snv_scaler.fit(train_p_dataset.snv_x[:, train_dataset.feature_indices])
                cnv_scaler.fit(train_p_dataset.cnv_x)

                val_dataset = ExSCNVDataset(
                    train_val_test_dataset[val_indices],
                    base_features=args.base_features,
                    additional_features=args.additional_features,
                    scalers=scalers,
                    device=args.device,
                ...
                log_msg = f"Fold {fold} Result \n"
                for name, metric_values in fold_metric_result.items():
                    log_msg += f"- {name}: {round(metric_values, 5)} \n"
        ...
        plot_cv_topk(fold_instance_y_trues, fold_instance_y_probs)
